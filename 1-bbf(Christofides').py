# -*- coding: utf-8 -*-
"""
Created on Sat Nov  9 10:16:06 2024
...
for S1 in S_list:
    ...
    for s in seeds:
        ...
        for i in range(order_n):
            ...
            temp7 = [location[k] for k in temp2]

            for item in temp7:
                order[i][item] = temp7.count(item)
                
            #统计每个订单的原始通道个数    
            temp3 = [aisle[k-1] for k in temp7]
        ...
        while (S > 0):
            ...
            if S >= 0:
                for i in range(C[1]):
                    d_f+=distance[0][tour[m][C[0]]]*order[m][tour[m][C[0]]]
                    del tour[m][C[0]]
                ...
                if tour[m] == [0, 0]:
                    ...
                    for j in range(len(tour[m])-2, 0, -1):  # 组合个数 n+...+1
                        ...
                        for k in range(j):
                            temp = 0
                            for r in range(long+1):
                                temp = temp + distance[tour[m][k+r]][tour[m][k+r+1]]
